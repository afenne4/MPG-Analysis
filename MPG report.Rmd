...
This anova table shows that since the p-value is less than .05, the second model with
cylinders and engine type improves upon the original model with just transmission type.
Furthermore, the third model improves upon the second one as the p-value for that 
comparison is also less than .05. Thus, including weight and horsepower in addition
to the predictors from second model seems to explain more of the variance in mpg,
than in the model that excludes these weight and horsepower.
...
as expected given the p-value is less than .05. 
# Appendix
